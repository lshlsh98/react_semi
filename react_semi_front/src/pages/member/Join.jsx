...
const Join = () => {
  ...
  const idDupCheck = () => {
    ...
    axios
      .get(`${import.meta.env.VITE_BACKSERVER}/members/exists?memberId=${member.memberId}`)
      .then((res) => {
        if (res.data) { 
          setCheckId(2);
        } else {
  ...
  const joinMember = () => {
    if (checkId !== 2) { alert("아이디 중복 체크를 해주세요."); return; }
    if (checkPw !== 1) { alert("비밀번호 일치 확인을 해주세요."); return; }
    if (member.memberName === "") { alert("이름(닉네임)을 입력하세요."); return; }
    if (mailAuth !== 3) { alert("이메일 인증을 완료해주세요."); return; }
    if (member.memberPostcode === "" || member.memberDetailAddr === "") { alert("주소를 입력해주세요."); return; }
          ...
          <div className={styles.form_group}>
            ...
            <div className={styles.input_row}>
              <Input
                ...
                onBlur={idDupCheck}
              />
              <Button type="button" className="btn primary sm" onClick={idDupCheck}>
                아이디 중복 체크
              </Button>
            </div>
            {checkId > 0 && (
              <p className={`${styles.validation_msg} ${checkId === 2 ? styles.valid : styles.invalid}`}>
                {checkId === 2 ? "사용 가능한 아이디 입니다." : "이미 사용중인 아이디 입니다."}
              </p>
            )}
          </div>
              ...
              <div className={styles.form_group}>
                <label htmlFor="memberPwRe" className={styles.label}>비밀번호 확인</label>
                <div className={styles.input_row}>
                  <Input
                    type="password"
                    name="memberPwRe"
                ...
                {checkPw > 0 && (
                  <p className={`${styles.validation_msg} ${checkPw === 1 ? styles.valid : styles.invalid}`}>
                    {checkPw === 1 ? "비밀번호가 일치합니다." : "비밀번호가 일치하지 않습니다."}
                  </p>
                )}
              </div>

              <div className={styles.form_group}>
                <label htmlFor="memberName" className={styles.label}>이름 (닉네임)</label>
                <div className={styles.input_row}>
                  <Input
                    type="text"
                    name="memberName"
              ...
              <div className={styles.form_group}>
                <label htmlFor="memberEmail" className={styles.label}>이메일 (E-Mail)</label>
                <div className={styles.input_row}>
                  <Input
                    type="email"
                    name="memberEmail"
              ...
              {mailAuth > 1 && (
                <div className={styles.form_group}>
                  <label htmlFor="mailAuthInput" className={styles.label}>이메일 (E-Mail) - 인증코드</label>
                  <div className={styles.input_row}>
                    <Input
                      type="text"
                      name="mailAuthInput"
                  ...
                  <p className={`${styles.validation_msg} ${mailAuth === 3 ? styles.valid : styles.invalid}`}>
              ...
              <div className={styles.form_group}>
                <label htmlFor="memberPostcode" className={styles.label}>주소</label>
                <div className={styles.input_row}>
                  <Input
                    type="text"
                    name="memberPostcode"
